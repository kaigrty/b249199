...
prescriptions_totals %>% 
  ggplot(aes(hb_name, total_prescriptions)) +
  geom_col() +
  labs(x = "Healthboard", y = "Total antidepressant prescriptions", title = "Antidepressant prescriptions by healthboard")
```
...
Data is available at: <https://www.scotlandscensus.gov.uk/search-the-census#/>
...
Now, we can display the antidepressant prescriptions alongside the education data. We'll do this by using the percentage of each healthboard's population that are degree holders we calculated earlier.
