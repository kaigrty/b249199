...
```{r message=FALSE, warning=FALSE}
graph1 <- ggplot(education_antidepressants, aes(x = degree_pct, y = apc, fill = hb_name)) +
  geom_point(size = 3) +
...
graph2 <- ggplot(antidepressants_education_dz, aes(x = degree_pct, y = apc)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(
    x = "Degree holders (%)",
